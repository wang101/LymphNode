...
NUM_OF_CLASSESS = 2
IMAGE_SIZE = 224
GRAY_MODE = True  #len(in_channel) = 1

def vgg_net(weights, image):
    layers = (
        'relu1_1', 'conv1_2', 'relu1_2', 'pool1',
...
def inference(image, keep_prob):
    ...
    #for kkk in model_data.keys():
    #    print(kkk)
    #    print(type(model_data[kkk]))
    #    if kkk=='layers' or kkk=='classes' or kkk =='normalization':
    #        print(model_data[kkk].shape)

    mean_pixel = np.mean(mean, axis=(0, 1))

    weights = np.squeeze(model_data['layers'])

    processed_image = utils.process_image(image, mean_pixel[0])
